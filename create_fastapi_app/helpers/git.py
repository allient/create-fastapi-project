...
def is_in_git_repository() -> bool:
    try:
        subprocess.run(['git', 'rev-parse', '--is-inside-work-tree'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        return True
...
def is_default_branch_set() -> bool:
    try:
        subprocess.run(['git', 'config', 'init.defaultBranch'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        return True
    except:
        pass
    return False
...
def try_git_init(root: str) -> bool:
    ...
    try:
        ...
        if not is_default_branch_set():
            subprocess.run(['git', 'checkout', '-b', 'main'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)

        subprocess.run(['git', 'add', '-A'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        subprocess.run(['git', 'commit', '-m', 'Initial commit from Create Fastapi App'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        return True
    except:        
        if did_init:
            try:
